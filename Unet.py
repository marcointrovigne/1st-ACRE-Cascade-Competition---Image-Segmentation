"""function to build Unet network"""

from tensorflow.keras.models import Model
from tensorflow.keras.layers import Input, Dense, Activation,\
    BatchNormalization, Dropout, MaxPooling2D,\
    Conv2D, concatenate, Conv2DTranspose, UpSampling2D
...
def build_decoder_block_up(previous_layer, skip_layer, is_last, filters, activation, use_batchnorm, dropout):
...
def build_Unet(
    ...
    Parameters:
        ...
        use_upsampling (bool):
            If True, use UpSampling2D layer on the decoder_blocks instead of Conv2DTranspose
    Returns:
        model (keras.model)
    """
    encoder_blocks = []
    ...
    for decoding_index in range(depth):
        ...
        if use_upsampling:
          build_dec = build_decoder_block_up
        else:
          build_dec = build_decoder_block

        decoder_block = build_dec(
            previous_layer,
            skip_layer,
